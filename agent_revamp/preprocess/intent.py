...
class LLMIntentClassifier(IntentClassifier):
    ...
    The OpenAI-compatible client is constructor-injected (mirrors
    core/vector.py's Embedder pattern) rather than reached for via a module-level
    singleton in agent.py, so this module has no dependency on agent.py — avoids a
    circular import now that agent.py imports from this package.
    """

    def __init__(self, client: Any, taxonomy: str | None = None):
        self._client = client
